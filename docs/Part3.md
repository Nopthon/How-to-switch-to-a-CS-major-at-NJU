...
??? failure "课程号为 1 开头的数学课（匡亚明学院开设的）不应该主动用于转专业，你应该也无法选中这门课"
    ...
    **<u>除非你已经在学习匡院微积分/线性代数了</u>，否则不要选课程号为 1 开头的数学课**
    
    课程名相同不代表课程号相同，**你在选课的时候应该永远以课程号相同为选课正确的标准**
    
    虽然说有匡院数学课转cs的先例，但是你不应该尝试，至少你得亲自问一问可行性

??? warning "有关“匿名专家”？"

    一层次微积分、线性代数的难度从 25Spring 开始出现明显的难度上升（体现在考试），并且不再和往年卷有较强的拟合关系
...
主要的教学参考用书是《程序设计教程：用C++语言编程》（前五章，不涉及 C++ 特性的内容），也会用到《C语言程序设计：现代方法》（比上一本书好）

期中、期末考试为笔试，期间穿插三次机试，学期末有一次大作业，平时每周会有几道上机题当作业（上机课的大多数时间都是用来写作业的）

<del>笔试机试难度都适中偏友好，有基础的比较容易 95+，无基础的 90+ 也不难（吧）</del>

机试难度中等，笔试因为会考比较细的 C 语言知识点（尤其是解答题）需要额外准备，可能写简答题之类的会比较难受

??? summary "主观的个人建议"

    这门课和《计算机程序的构造和解释》是二选一关系（也可以都选，但是会比较累）
    
    期中/期末考试都是笔试，而且有简答题这种题型。零基础的学生可能需要注意一些知识点细节（比如可能会问“冯诺依曼计算机体系结构的构成”），而不仅仅是会敲代码
    
    不过听说这门课在改革，比如笔试的题目类型
    
    **这门课一定不要依赖 AI！！所有的代码必须亲手实现！！**
    
    > 记得是《程序设计基础》的某一次上机测试，据说场上得分率低的吓人，但是在我们（指转cs群学长）看到实际的考题后，发现这大概率不是因为题目难度过大，而是因为小登不够熟练，sad
...
总体来说这是很优质的一门课，就是**对纯cs新手真的不太友好，真的有难度**

给个参考文献：[计算机程序的构造与解释 - ICS Wiki](https://njuics-wiki.github.io/ics-wiki/Course/SICP/)
...
??? summary "主观的个人建议"
    ...
    自学 CS61A 也是可以的，或者大二申请校内 OJ 账号结合 CS61A 网课跟着自学（？
...
（多数情况）每周两个 Problem Set（也就是作业），每个 Problem Set 平均十道题（有波动），会有专门的习题课讲解题目（每周一次）；期中 期末考试占比分别是 20% 60%，难度不至于太难，给分相对友好，但如果不认真学以至于期末考试直接空题也不会得到一个满意的分数（因为这就是我本人）
...
主要的教学参考用书是《数字逻辑与计算机组成》（及其配套的“习题解答与实验教程”），主要用到的软件是仿真电路软件 Logisim
...
??? summary "主观的个人建议"
    ...
    令人感慨，一年后我成了这门课的助教，发现自己这门课真的跟没学过一样
...
!!! warning "和往年不一样的是"
    ...
    **高级程序设计从“与大二的《概率论与数理统计》二选一”修改为了必修，所以你现在终究会选这门课的**
    ...
    即使转专业准入要求还是 n 选一专业课，但是准出毕业需要这门课
